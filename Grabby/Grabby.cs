...
using Grabby.UI.Windows;
using Module = ECommons.Module;

namespace Grabby;

public sealed partial class Grabby : IDalamudPlugin
{
    private const string CommandName = "/grabby";
    private readonly ConcurrentQueue<Action> _mainThreadActions = new();
    private readonly Luna.ImSharpDalamudContext _imSharpContext;

    public Grabby(IDalamudPluginInterface pluginInterface)
    {
        ...
        SnapshotFS = new Luna.BaseFileSystem("GrabbySnapshots", Log, false);
        ...
        Svc.Commands.AddHandler(
            CommandName,
            new CommandInfo(OnCommand) { HelpMessage = "Opens main Grabby interface" }
        );
    ...
    public WindowSystem WindowSystem { get; } = new("Grabby");
