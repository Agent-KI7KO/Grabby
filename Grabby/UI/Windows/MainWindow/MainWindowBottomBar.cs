...
{
    public static void Draw(Grabby grabby, IActiveSnapshotManager activeSnapshotManager)
    {
        var workingDirectory = grabby.Configuration.WorkingDirectory;
        ...
        if (
            ...
            grabby.FileDialogManager.OpenFolderDialog(
                "Where do you want to save your snaps?",
                (status, path) =>
                {
                    if (!status || string.IsNullOrEmpty(path) || !Directory.Exists(path))
                        return;
                    grabby.Configuration.WorkingDirectory = path;
                    grabby.Configuration.Save();
                    Notify.Success("Working directory updated.");
                    grabby.InvokeSnapshotsUpdated();
                }
            );
