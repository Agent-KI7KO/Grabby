    ...
    private void DrawPlayerSelector()
    {
        ...
        )
        {
            ...
            else
            {
                ...
                var useLiveData = _grabby.Configuration.UseLiveSnapshotData || isLocalPlayer;
                var useIpcResourcePaths = useLiveData && _grabby.Configuration.UsePenumbraIpcResourcePaths;
                ...
                _grabby.ExecuteBackgroundTask(async () =>
                {
                    ...
                    if (updatedSnapshotPath != null)
                        _grabby.QueueAction(() => _grabby.InvokeSnapshotsUpdated());
                });
            }
        }
        ...
        using (var disabled = ImRaii.Disabled(isLockDisabled))
        {
            using (var font = ImRaii.PushFont(UiBuilder.IconFont))
            {
                if (Im.Button(lockIcon.ToIconString(), lockButtonSize))
                    if (_isActorLockedByGrabby && TryGetSelectedActor(out var selectedActor))
                    {
